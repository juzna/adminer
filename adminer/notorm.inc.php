...
$db = new NotORM($connection, new NotORM_Structure_Discovery($connection, new NotORM_Cache_Session, "%sId"));
...
restart_session();
$history_all = &get_session("notorm-queries");
...
page_header(lang('NotORM query'), $error, array(), "", "<link rel='stylesheet' href='$codemirror_path/lib/codemirror.css'>");

if (!$error && $_POST) {
	$fp = false;
	$query = $_POST["query"];
	if ($query) {
		$query = eval("return ${query}->__toString();");
...
textarea("query", $q ?: '$db->myTable()', 20, 80, "query");
