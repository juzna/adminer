...
ORDER BY a.attnum"
	...
	function foreign_keys($table) {
		...
		foreach (get_rows("SELECT oid, conname, (SELECT nspname FROM pg_namespace WHERE oid = connamespace) AS ns, pg_get_constraintdef(oid) AS definition
			FROM pg_constraint
...
WHERE tc.constraint_name NOT LIKE '$%' AND tc.constraint_type = 'FOREIGN KEY' AND ccu.table_name = " . q($table) //! there can be more unique_constraint_name
			) as $row) {
